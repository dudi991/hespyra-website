...
const Purchase = () => {
  ...
  return (
    <section id="purchase" className="w-full bg-[#333230] py-12 sm:py-16 lg:py-32 relative z-10 text-[#F8F5EE] overflow-hidden">
      ...
      <div className="max-w-[1600px] mx-auto px-6 sm:px-8 lg:px-12 text-center relative z-10">
        ...
        {/* Section Tag */}
        <span className="font-sans text-[10px] sm:text-[11px] tracking-[0.25em] font-medium text-[#C98A5F] uppercase mb-4 block select-none">
          {tagText}
        </span>
        
        {/* Headline */}
        <h2 className="text-[23px] sm:text-4xl md:text-5xl leading-[1.25] font-serif text-[#F8F5EE] tracking-tight font-normal mb-6">
          {titleText}
        </h2>

        {/* Subline */}
        <p className="font-sans text-[13px] sm:text-base text-[#F8F5EE]/80 leading-relaxed font-light max-w-2xl mx-auto mb-8">
          ...
          {submitted ? (
            ...
            <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-3 w-full">
              <input 
                ...
                className="flex-1 bg-[#292827] border border-[#F8F5EE]/10 text-[#F8F5EE]/90 px-5 py-4 text-xs font-sans tracking-wide focus:outline-none focus:border-accent transition-colors font-light placeholder-[#E6DFD4]/70 rounded-sm"
                required
                autoComplete="off"
              />
              <button 
        ...
        {/* Microcopy */}
        <p className="font-sans text-[13px] sm:text-[14px] text-[#F8F5EE]/80 tracking-wide mb-6 md:mb-10 select-none">
          {language === 'de' 
        ...
        {/* Trust Info Points */}
        <div className="flex flex-col sm:flex-row items-center justify-center gap-y-2 sm:gap-y-4 gap-x-12 sm:gap-x-16 text-[#F8F5EE]/80 font-sans text-[11.5px] sm:text-[14px] tracking-widest uppercase font-normal mt-4 md:mt-10">
